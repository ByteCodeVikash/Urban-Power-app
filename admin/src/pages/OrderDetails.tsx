...
import {
  Box, Card, CardContent, Typography, Grid, Button, Divider,
  Stepper, Step, StepLabel, StepContent, Skeleton, Alert,
...
import { useAdminOrderDetail, useAdminOrderStatusUpdate } from '../hooks/useAdminOrders';
...
const STATUS_LABEL: Record<string, string> = {
  pending: 'Pending', requested: 'Requested', accepted: 'Accepted',
  confirmed: 'Confirmed', assigned: 'Assigned',
  technician_on_the_way: 'On The Way', reached: 'Reached',
  work_started: 'Work Started', in_progress: 'In Progress',
  ...
  <Box sx={{ flexGrow: 1 }}>
    ...
    <Grid container spacing={3}>
      <Grid size={{ xs: 12, md: 7 }}>
        {[0, 1, 2].map(i => (
          <Skeleton key={i} variant="rectangular" height={160} sx={{ borderRadius: 3, mb: 3 }} />
        ))}
      </Grid>
...
const DetailCard: React.FC<{ title: string; icon?: React.ReactNode; children: React.ReactNode }> = ({
  title, icon, children,
}) => (
  <Card sx={{
    borderRadius: '16px',
    ...
    <CardContent sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2.5 }}>
        {icon}
        <Typography variant="h6" sx={{ fontWeight: 700, fontFamily: '"Outfit",sans-serif', color: '#2D3748' }}>
          {title}
        </Typography>
      </Box>
      {children}
...
export const OrderDetails: React.FC = () => {
  const { bookingType, bookingId, id } = useParams<{
    bookingType?: string; bookingId?: string; id?: string;
  }>();
  const navigate = useNavigate();

  // Support both /orders/:bookingType/:bookingId and legacy /orders/:id
  const resolvedType = bookingType as 'beautician' | 'scrap' | 'maintenance' | null | undefined;
  ...
  if (!hasFullParams) {
    return (
      <Box sx={{ flexGrow: 1 }}>
        <Button variant="outlined" color="secondary" startIcon={<BackIcon />}
          onClick={() => navigate('/orders')} sx={{ mb: 3 }}>
          Back to Orders
        </Button>
        <Alert severity="warning">
          This link is missing the booking type. Please navigate from the Orders list.
        </Alert>
      </Box>
    );
  }
  ...
  const typeColors = BOOKING_TYPE_COLORS[order.booking_type] || BOOKING_TYPE_COLORS.beautician;
  ...
  return (
    <Box sx={{ flexGrow: 1, pb: 6 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Ref:{' '}
              <strong style={{ color: '#2B6CB0' }}>{order.booking_reference}</strong>
              {' · '}
              <Chip label={order.booking_type} size="small"
                sx={{ fontWeight: 700, fontSize: '0.7rem', bgcolor: typeColors.bg, color: typeColors.color }} />
            </Typography>
          </Box>
        </Box>
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          <Button variant="outlined" startIcon={<TechIcon />} onClick={handleOpenAssign}
            sx={{ borderRadius: '12px', textTransform: 'none', fontWeight: 600 }}>
            {order.assigned_technician ? 'Change Technician' : 'Assign Technician'}
          </Button>
          <Button variant="contained" color="primary" startIcon={<EditIcon />} onClick={handleOpenStatus}
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
        <Box sx={{ px: 2.5, py: 1, borderRadius: 3, bgcolor: statusColors.bg, color: statusColors.color,
          fontWeight: 800, fontSize: '0.9rem', fontFamily: '"Outfit",sans-serif' }}>
          {STATUS_LABEL[normStatus] || order.status}
        </Box>
        <Box sx={{ px: 2.5, py: 1, borderRadius: 3, bgcolor: '#F7FAFC', fontWeight: 600,
          fontSize: '0.85rem', color: '#4A5568' }}>
          ₹{Number(order.price || 0).toLocaleString('en-IN')}
        </Box>
        {order.assigned_technician && (
          <Box sx={{ px: 2.5, py: 1, borderRadius: 3, bgcolor: 'rgba(102,126,234,0.1)',
            fontWeight: 600, fontSize: '0.85rem', color: '#553C9A', display: 'flex', alignItems: 'center', gap: 0.5 }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          ...
          {/* Customer Info */}
          <DetailCard title="Customer Information" icon={<CustomerIcon sx={{ color: '#553C9A' }} />}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>{order.customer_email || '—'}</Typography>
                  </Box>
                </Box>
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ fontWeight: 600 }}>{order.address || '—'}</Typography>
                  </Box>
                </Box>
              </Grid>
            </Grid>
          ...
          {/* Booking Info */}
          <DetailCard title="Booking Information" icon={<ReceiptIcon sx={{ color: '#2B6CB0' }} />}>
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, sm: 6 }}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <InfoRow label="BOOKING DATE" value={fmtDate(order.booking_date)} />
              </Grid>
              <Grid size={{ xs: 12, sm: 6 }}>
              ...
              {order.booking_type === 'scrap' && (
                <>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <InfoRow label="ESTIMATED WEIGHT" value={order.estimated_weight_kg ? `${order.estimated_weight_kg} kg` : undefined} />
                  </Grid>
                  <Grid size={{ xs: 12, sm: 6 }}>
                ...
                <Grid size={12}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {order.service_names.map((sn, i) => (
                      <Chip key={i} label={sn} size="small" variant="outlined" />
                    ))}
                  </Box>
                </Grid>
              )}
              {order.notes && (
                <Grid size={12}>
                  ...
                  <Typography variant="body2" sx={{
                    bgcolor: '#F7FAFC', p: 1.5, borderRadius: 2,
                    borderLeft: '3px solid #CBD5E0', color: '#4A5568',
                  }}>
                    {order.notes}
                  </Typography>
                </Grid>
          ...
          <DetailCard title="Technician Assignment" icon={<TechIcon sx={{ color: '#D69E2E' }} />}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2 }}>
              <Box>
                <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 0.5 }}>
                  ASSIGNED TECHNICIAN
                </Typography>
                <Typography variant="body1" sx={{ fontWeight: 700, color: order.assigned_technician ? '#2D3748' : '#A0AEC0' }}>
                  {order.assigned_technician || 'Not yet assigned'}
                </Typography>
              </Box>
              <Box sx={{ display: 'flex', gap: 1 }}>
                <Button variant="contained" color="warning" startIcon={<TechIcon />}
        ...
        <Grid size={{ xs: 12, md: 5 }}>
          ...
          {/* Status Timeline */}
          <Card sx={{ borderRadius: '16px', border: '1px solid rgba(226,232,240,0.8)',
            boxShadow: '0 4px 20px -2px rgba(148,163,184,0.08)', mb: 3 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              {order.status_history && order.status_history.length > 0 ? (
                      ...
                      <StepLabel
                        icon={
                          <Box sx={{
                            ...
                            <DoneIcon sx={{ fontSize: 16, color: idx === 0 ? 'white' : '#A0AEC0' }} />
                          </Box>
                        }
                      >
                        <Typography variant="body2" sx={{ fontWeight: 700, color: '#2D3748' }}>
                          {STATUS_LABEL[entry.status?.toLowerCase()] || entry.status}
                        </Typography>
                      </StepLabel>
                      <StepContent>
                        ...
                        {entry.updated_by_name && (
                          <Typography variant="caption" sx={{ color: '#553C9A', fontWeight: 600 }}>
                ...
                <Alert severity="info" sx={{ borderRadius: 2 }}>
                  No status changes recorded yet. The first update will appear here.
                </Alert>
              )}
            </CardContent>
          </Card>
      ...
      <Dialog open={openStatusDialog} onClose={() => setOpenStatusDialog(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          ...
          <FormControl fullWidth size="small" sx={{ mb: 2 }}>
            ...
            <Select value={orderStatus} label="Select Status"
      ...
      <Dialog open={openAssignDialog} onClose={() => setOpenAssignDialog(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Assign a field professional to <strong>{order.booking_reference}</strong>.
          </Typography>
          <FormControl fullWidth size="small">
            <InputLabel>Select Technician</InputLabel>
            <Select value={assignedTech} label="Select Technician"
        ...
        <DialogActions sx={{ p: 2.5 }}>
          <Button onClick={() => setOpenAssignDialog(false)} color="secondary">Cancel</Button>
