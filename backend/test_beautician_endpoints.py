...
from app.core.seeding import seed_beautician_data
...
def db_session():
    ...
    # Pre-seed the beautician data for testing since the DB tables are transactional
    seed_beautician_data(session)
...
def test_get_categories_success(client, db_session):
    """
    Tests that we can successfully fetch seeded beautician categories.
    """
    response = client.get("/api/v1/beautician/categories")
...
def test_get_services_success(client, db_session):
    """
    Tests fetching all services.
    """
    response = client.get("/api/v1/beautician/services")
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
...
def test_get_services_filtered_by_category(client, db_session):
    """
    Tests fetching services filtered by category_id.
    """
    ...
    response = client.get(f"/api/v1/beautician/services?category_id={cat_id}")
    assert response.status_code == 200
    data = response.json()
...
def test_get_services_empty_category(client, db_session):
    ...
    response = client.get(f"/api/v1/beautician/services?category_id={random_uuid}")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 0
