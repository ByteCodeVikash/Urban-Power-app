...
export default function MaintenanceServicesScreen() {
  ...
  const route = useRoute<MaintenanceServicesRouteProp>();
  ...
  const { data: services, isLoading: loadingServices } =
    useMaintenanceServices(selectedCatId);
  ...
  const handleServiceToggle = (service: MaintenanceService) => {
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable
                style={styles.card}
                onPress={() =>
                  navigation.navigate('MaintenanceServiceDetails', {
                    serviceId: item.id,
